...
# next build imports every route module, and src/lib/db throws without a connection string, so the
# build needs one even though it never opens it. Scoped to this command so neither placeholder is
# recorded in the image; the real values are read from the environment at runtime.
