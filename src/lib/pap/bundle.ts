...
export function encodePapBundle(files: PapFile[]): Uint8Array {
  const header = encodePapBundleHeader(files.map((file) => ({ path: file.path, length: file.data.byteLength })))
  const payloadBytes = files.reduce((total, file) => total + file.data.byteLength, 0)
  const bundle = new Uint8Array(header.length + payloadBytes)

  bundle.set(header, 0)
  let offset = header.length
  for (const file of files) {
    bundle.set(new Uint8Array(file.data), offset)
...
export function decodePapBundle(bundle: ArrayBuffer): PapFile[] {
  ...
  const files: PapFile[] = []
  let offset = HEADER_BYTES + manifestBytes
  for (const entry of entries) {
    files.push({ path: entry.path, data: bundle.slice(offset, offset + entry.length) })
    offset += entry.length
  }
...
function parseManifest(text: string): PapBundleEntry[] {
  ...
  return parsed.map((entry) => {
    ...
    return { path: candidate.path, length: candidate.length as number }
  })
}
